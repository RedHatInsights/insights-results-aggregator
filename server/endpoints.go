...
const (
	...
	RuleEndpoint = "organizations/{org_id}/clusters/{cluster}/users/{user_id}/report/{rule_id}"
...
func (server *HTTPServer) addEndpointsToRouter(router *mux.Router) {
	...
	router.HandleFunc(apiPrefix+RuleEndpoint, server.readRuleForReport).Methods(http.MethodGet, http.MethodOptions)
